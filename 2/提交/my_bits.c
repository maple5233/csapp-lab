...
int negate(int x) {
	//求相反数即取反+1
	return ~x + 1;//当然这里有个漏洞 就是0x80000000(Tmin)的相反数是不能这么算的 因为0x80000000(Tmin)的相反数无法用int型表示 
	//Tmin取反加1后还是它自己
}
//3
/*
...
int isLessOrEqual(int x, int y) {
	...
	int comp = !(!((x + ~y) >> 31));//双重!强制转为0或1; 这里是用x+~y而不是x+(~y+1)是因为这是x<=y而不是x<y ~y相当于y的相反数减去1
...
unsigned float_i2f(int x) {
	...
	if (x << 1) //x不是0也不是Tmin;
	{
		...
		/** 计算尾数并使其归位 **/
		x = x << (31 - tmp);//把x左边的零全部移到右边;方便观察第24-32位
		...
		x = x & 0x000000FF;//只保留后八位;尾数长达24-32位时,这个值将是尾数的最后几位;IEEE754标准的尾数只有23位最长;
